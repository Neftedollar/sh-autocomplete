...
pub mod triggers_for_test {
    use super::Context;
    pub fn git_branches(c: &Context) -> bool {
        super::triggers::git_branches(c)
    }
    pub fn ssh_hosts(c: &Context) -> bool {
        super::triggers::ssh_hosts(c)
    }
    pub fn npm_scripts(c: &Context) -> bool {
        super::triggers::npm_scripts(c)
    }
    pub fn make_targets(c: &Context) -> bool {
        super::triggers::make_targets(c)
    }
    pub fn docker_images(c: &Context) -> bool {
        super::triggers::docker_images(c)
    }
    pub fn transitions(c: &Context) -> bool {
        super::triggers::transitions(c)
    }
    pub fn unknown_command(c: &Context) -> bool {
        super::triggers::unknown_command(c)
    }
    pub fn hybrid_cd(c: &Context) -> bool {
        super::triggers::hybrid_cd(c)
    }
    pub fn kubectl_resources(c: &Context) -> bool {
        super::triggers::kubectl_resources(c)
    }
    pub fn path_jump_cyan(c: &Context) -> bool {
        super::triggers::path_jump_cyan(c)
    }
}
...
pub mod storage {
    ...
    pub fn reset(conn: &Connection, hard: bool) -> Result<()> {
        if hard {
            conn.execute("DELETE FROM tips_state", [])
                .context("reset --hard")?;
        } else {
            conn.execute(
...
pub fn select<'a>(input: &'a SelectInput<'a>) -> Option<&'static Tip> {
    ...
    let mut candidates: Vec<&'static Tip> = catalog()
        .iter()
        .filter(|t| {
            if !(t.trigger)(input.context) {
                return false;
            }
            if input.session.shown_this_session.contains(t.id) {
                return false;
            }
    ...
    candidates.sort_by_key(|t| {
        ...
        let last_shown = input
            .state
            .get(t.id)
            .and_then(|s| s.last_shown_at)
            .unwrap_or(0);
